...
public class TowerController : MonoBehaviour
{
    ...
    {
        if (_towers.Length < LevelDataLoader.LevelUnlockedCount) _isAllRepair = true;
        if (!_isAllRepair)
        {
            ...
            for (int i = 0; i < LevelDataLoader.LevelUnlockedCount; i++)
            {
            ...
            _towers[LevelDataLoader.LevelUnlockedCount - 1].transform.GetChild(0).GetComponent<Renderer>().material = _unlockedMaterial;
            _towers[LevelDataLoader.LevelUnlockedCount - 1].IsComplete = false;
        }
    }
    private void Init()
    {
